...
const SelecccionarHora = ({
  ...
  return (
    <select className={className} onChange={changeHora} value={horaSelec}>
...
const AgendarTurno = () => {
  ...
  const horasDisponiblesParaTurno = (horasOcupadas) => {
    ...
    const horasDisponibles = horasTotales.filter(
      (hora) => !conjuntoHorasOcupadas.has(hora)
    ); //filtramos el array segun los elementos que NO estan en el conjunto
  ...
  const getTurnos = async () => {
    ...
    const q = query(
      ...
      where("__name__", ">=", fechaActualParsed)
    );
    ...
    if (data.docs.length !== 0) {
      ...
      data.docs.forEach((doc) => {
        turnos[doc.id] = doc.data().diaOcupado ? "OCUPADO" : "DISP";
      });
  ...
  const getHorasReservadas = async (dia) => {
    ...
    const turnosDelDia = await getDocs(
      collection(db, `Turnos/${diaTurno}/TurnosDelDia`)
    );
    if (!turnosDelDia.empty) {
      setHorasOcupadas(turnosDelDia.docs.map((doc) => doc.id));
    } else {
      setHorasOcupadas([]);
  ...
  const cargarTurno = async () => {
    ...
    if (turnos[diaTurno] !== undefined) {
      ...
      if (horasDisponiblesParaTurno(horasOcupadas).length === 1) {
        ...
        await updateDoc(turnoRef, {
          ...
          console.log(
            "Algo fallo al agregar el nuevo turno - update dia ocupado ",
            error
          );
          return;
        });
      ...
      await setDoc(doc(db, "Turnos", `${diaTurno}`), {
        ...
        console.log(
          "Algo fallo al agregar el nuevo turno - creacion de doc dia",
          error
        );
        return;
      });
    ...
    await setDoc(docRef, {
      ...
      .catch((error) => {
        console.log(
          "Algo fallo al agregar el nuevo turno - setdoc hora",
          error
        );
        return;
      });
