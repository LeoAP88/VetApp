...
const construirMes = (mes, anio) => {
  ...
  for (let dia = 1; dia <= diasMes; dia++) {
    let dateStr = `${anio}-${IntADosCifrasStr(mes)}-${IntADosCifrasStr(
      dia
    )}T00:00:00.000-03:00`;
    if (!arrayDates[semana]) arrayDates[semana] = [];
  ...
  if (primerDiaMes != 0 || ultDiaMes != 6) {
    ...
    for (let i = 0; i < primerDiaMes; i++) {
      let dateStr = `${anioMesAnt}-${IntADosCifrasStr(
        mes - 1 || 12
      )}-${IntADosCifrasStr(diaMesAnt)}T00:00:00.000-03:00`;
    ...
    for (let i = 6; i > ultDiaMes; i--) {
      let dateStr = `${anioMesPost}-${IntADosCifrasStr(
        (mes + 1) % 12 || 1
      )}-${IntADosCifrasStr(diaMesSiguiente)}T00:00:00.000-03:00`;
...
const DiaCalendario = ({ onClick, dia, children, propsEstilo }) => {
  ...
  const onClickOverride = (e) => {
    if (propsEstilo.deshabilitado) return;

    onClick(e, dia);
  };

  const clasesEstilo = `${propsEstilo.isInh ? "table-danger" : ""} ${
    propsEstilo.activo ? "table-primary" : ""
  } ${propsEstilo.otroMes ? "table-secondary" : ""} ${
    propsEstilo.isAntAHoy ? "table-warning" : ""
  } dia-calendario`;

  return (
    <td className={clasesEstilo} onClick={onClickOverride}>
      {children}
    </td>
  );
};
...
const Calendario = ({ diasInhabilitados = [], diaActivo, toggleDia }) => {
  ...
  const diasDelMes = construirMes(
    diaActivo.getMonth() + 1,
    diaActivo.getFullYear()
  );
  ...
  const clickDia = (e, diaClick) => {
    ...
    const dateStr = `${anio}-${IntADosCifrasStr(mes)}-${IntADosCifrasStr(
      dia
    )}T00:00:00.000-03:00`;
  ...
  const clickMesAnt = () => {
    ...
    const dateStr = `${anio}-${IntADosCifrasStr(
      mes - 1 || 12
    )}-${IntADosCifrasStr(dia)}T00:00:00.000-03:00`;
    toggleDia(new Date(dateStr));
  };
  const clickMesPost = () => {
    let mes = diaActivo.getMonth() + 1;
  ...
  return (
    <div className="container_calendario">
      <div className="container d-flex flex-direction-row align-items-center justify-content-center">
        <button
          type="button"
          className="me-auto btn-calendario"
          onClick={clickMesAnt}
        >
          <BsArrowLeft />
        </button>
        <h3>
          <b>{`${MESES[diaActivo.getMonth()]} - ${diaActivo.getFullYear()}`}</b>
        </h3>
        <button
          type="button"
          className="ms-auto btn-calendario"
              ...
              return (
                <tr key={index}>
                  {semana.map((dia, index) => {
                    const isInh = diasInhabilitados.find(
                      (inh) => inh === dia.toISOString()
                    );
                    const isActivo =
                      diaActivo.toLocaleDateString() ===
